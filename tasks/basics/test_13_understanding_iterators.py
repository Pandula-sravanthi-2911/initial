...
def test_int_iterable():
    try:
        iter(10)
    except:  # replace by appropriate except so this test passes
        pass
...
def test_enumerate_iter():
    ...
    try:
        ...
        assert None == next(list_iter) #note what happens when items are finished.
...
def test_find_builtins_that_work_on_iterables():
    funcs = ['all','any','filter','frozenset','list','map','max','min','next','reduce','set','sum','tuple','zip']
    assert 14== len(funcs)
