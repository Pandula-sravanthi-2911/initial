...
def test_classes_are_object_factories():
    ...
    assert type(q1).__class__ == type(q1).__class__
    assert type(q2).__class__== type(q2).__class__
    try:
...
def test_classes_bound_and_unbound_methods():
    ...
    try:
        assert False== Queue.push.__self__
        assert True== q1_push.__self__ #unbound method
